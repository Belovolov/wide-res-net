...
init_shape = (32, 32, 3)
file_model = 'CIFAR10-WRN-{}-{}' .format(((N*6)+4), k)
...
print('\n===== Training a Wide Residual Network of {} depth and {} width using CIFAR-10 dataset for {} epochs =====\n' .format(((N*6)+4), k, nb_epoch))

model_json = model.to_json()
with open(file_model + '.json', 'w') as json_file:
    json_file.write(model_json)

callbacks = [ LearningRateScheduler(schedule=lrschedule), ModelCheckpoint('results/CIFAR10.weights.{epoch:02d}-{val_acc:.2f}.hdf5', monitor='val_acc', verbose=1, save_best_only=True, mode='auto')]
