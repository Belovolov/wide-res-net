...
model = wrn.create_wide_residual_network(init_shape, nb_classes=10, N=4, k=10, dropout=0.3)
...
with open("10wrn-28-10.json", "w") as json_file:
...
callbacks = [ LearningRateScheduler(partial(getitem, tuple(take(nb_epoch, concat(repeat(0.1, 60), repeat(0.02, 60), repeat(0.004, 40), repeat(0.0008)))))),
          ModelCheckpoint('results/weights.{epoch:02d}-{val_acc:.2f}.hdf5', monitor='val_acc', verbose=1, save_best_only=True, mode='auto'),
...
model.save_weights("10wrn-28-10.h5")
